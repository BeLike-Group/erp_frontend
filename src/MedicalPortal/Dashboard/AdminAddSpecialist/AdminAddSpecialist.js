...
const AddSpecialist = () => {
  ...
  useEffect(() => {
    ...
    const fetchSpecialists = async () => {
      try {
        const response = await axios.get("/api/v1/admin/load-all-specialists");
  ...
  const onSubmit = async (data) => {
    try {
      setLoading(true);
      const response = await axios.post("/api/v1/admin/add-specialist", data);
  ...
  const handleDelete = async (id) => {
    try {
      setLoading(true);
      const response = await axios.delete(`/api/v1/admin/delete-specialist/${id}`);
      handleShowSuccessToast(response.data.message);
  ...
  const handleUpdate = async (id, updatedData) => {
    try {
      setLoading(true);
      const response = await axios.put(`/api/v1/admin/update-specialist/${id}`, updatedData);
      handleShowSuccessToast(response.data.message);
