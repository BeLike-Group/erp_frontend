...
const AddPharmacy = () => {
  ...
  useEffect(() => {
    ...
    const fetchPharmacies = async () => {
      try {
        const response = await axios.get("/api/v1/admin/load-all-pharmacies");
  ...
  const onSubmit = async (data) => {
    try {
      setLoading(true);
      const response = await axios.post("/api/v1/admin/add-pharmacy", data);
  ...
  const handleDelete = async (id) => {
    try {
      setLoading(true);
      const response = await axios.delete(`/api/v1/admin/delete-pharmacy/${id}`);
      handleShowSuccessToast(response.data.message);
  ...
  const handleUpdate = async (id, updatedData) => {
    try {
      setLoading(true);
      const response = await axios.put(`/api/v1/admin/update-pharmacy/${id}`, updatedData);
      handleShowSuccessToast(response.data.message);
