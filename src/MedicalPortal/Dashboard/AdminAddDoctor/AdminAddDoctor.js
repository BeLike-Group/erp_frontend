...
const AddDoctor = () => {
    ...
    const fetchHospitalsAndSpecializations = async () => {
      try {
        const [hospitalsResponse, specializationsResponse] = await Promise.all([
          axios.get("/api/v1/admin/load-all-hospitals"),
          axios.get("/api/v1/admin/load-all-specializations")
        ]);
  ...
  const onSubmit = async (data) => {
    try {
      setLoading(true);
      const response = await axios.post("/api/v1/admin/add-doctor", data);
  ...
  const handleDelete = async (id) => {
    try {
      setLoading(true);
      const response = await axios.delete(`/api/v1/admin/delete-doctor/${id}`);
      handleShowSuccessToast(response.data.message);
  ...
  const handleUpdate = async (id, updatedData) => {
    try {
      setLoading(true);
      const response = await axios.put(`/api/v1/admin/update-doctor/${id}`, updatedData);
      handleShowSuccessToast(response.data.message);
