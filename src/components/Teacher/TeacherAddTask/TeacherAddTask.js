    ...
    const loadCourses = async () => {
      try {
        const response = await axios.get(
          `/api/v1/teacher/load-courses-with-grade/${grade}`
        );
  ...
  const onSubmit = async (data) => {
    if (data) {
      try {
        ...
        formData.append("taskImage", data.taskImage[0]); // Append the first file from the file input
        formData.append("course", data.course);
        formData.append("grade", data.grade);
        formData.append("removeTime", data.removeTime);
