...
const AdminProfile = () => {
  ...
  const [adminDetails, setAdminDetails] = useState({
    ...
    announcement: currentAdminData?.admin?.announcement || "", // Added announcement field
  });
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await axios.put(
        `/api/v1/admin/updateadmin/${adminId}`, // Replace with your update profile API endpoint
        formData,
        {
          headers: {
    ...
    <div className="max-w-4xl mx-auto mt-10 bg-white shadow-md rounded-lg p-8">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2">
          ...
          <textarea
            name="announcement"
            value={adminDetails.announcement}
            onChange={handleInputChange}
            className="mt-1 p-3 text-black block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
            disabled={!isEditing}
          />
