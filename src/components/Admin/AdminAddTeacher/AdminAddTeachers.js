...
export const AdminAddTeacher = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const url = editingTeacher ? `/api/v1/admin/update-teacher/${editingTeacher._id}` : "/api/v1/admin/add-teacher";
      ...
      const teachersResponse = await axios.get("/api/v1/admin/load-all-teachers");
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`/api/v1/admin/delete-teacher/${id}`);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="relative">
              ...
              <input
                ...
                name="teacherPassword" value={formData.teacherPassword} onChange={handleInputChange} className="text-black p-2 mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
