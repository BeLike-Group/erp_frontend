...
const StudentFeeTracker = () => {
  ...
  const handleAmountReceivedChange = (e) => {
    ...
    setRemainingAmount(totalFee - received);
  };
  ...
  const handleSubmitFee = async () => {
    try {
      ...
      setShowPrintSlip(true); // Show PrintSlip after successful submission
    } catch (error) {
  ...
  const handlePrintComplete = () => {
    // Hide PrintSlip and show StudentFeeTracker again
    ...
    <div className="p-4 bg-gray-800 min-h-screen">
      ...
      {!showPrintSlip ? (
        ...
        <PrintSlip
          ...
          feeData={{ remainingAmount, totalFee, amountReceived, submissionDate }}
        />
      )}
    </div>
  );
